<template>
  <grid-layout ref='container' rows="auto, auto, *" class="background">
    ...
    <grid-layout row="0" columns="44, *, auto" class="action-bar-custom">
      ...
      <stack-layout col="2" @tap="showMenu()">
    ...
    <grid-layout row="1" columns="auto, *, auto" class="add-bar"
      ...
      <stack-layout class="add-bar-image-container" col="0" @tap="add('button')">
        <image col="0" [src]="isShowingRecent ? 'res://recent' : 'res://add'"></image>
      </stack-layout>
      <text-field ref='groceryTextField' col="1" v-model="grocery"
        @loaded="handleAndroidFocus()"
        :hint="isAndroid ? 'ADD A GROCERY' : 'Add a grocery'"
        returnKeyType="done" v-if="!isShowingRecent" @returnPress="add('textfield')"></TextField>
